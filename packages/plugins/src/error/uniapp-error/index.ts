...
function observeSyncError(reporter: ErrorReporter) {
  const Uni = (globalThis as any).__Monitor__Framework__;
  if (!Uni) {
    console.error('Uni instance not found, Make sure you are config frameworkInstance correctly.');
    return;
  }
...
function observePromiseError(reporter: ErrorReporter) {
  ...
  Uni.onUnhandledRejection(handler);
}

export class UniErrorPlugin extends BasePlugin<ErrorPluginOption> {
  name = PLUGIN_NAMES.UNI_ERROR;

  protected reportType = REPORT_TYPE.UNI_ERROR;
